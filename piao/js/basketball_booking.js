function initData(date = null) {
    ...
    // const data = { userName: readCookie("userName"), type: 'Basketball', useDate: date }
    const data = { type: 'Basketball', useDate: date }
...
async function addSlot(list) {
    var time = 8;
    var num = 0;
    var number = await findData();
    var vertical = document.getElementById("content");
    vertical.innerHTML = '';
    var vertical1 = document.getElementById("court-header");
    vertical1.innerHTML = '';
    let div = ``;
    ...
    if (number.available == '1'){
        for (let index = 0; index < 12; index++) {
            ...
            for (let lotIndex = 0; lotIndex < number.court; lotIndex++) {
                ...
                let available = 2
                content += `<div class="${className}" click="timeslot()" data-time="${time}:00" num="${num}" available="${available}" data-court="${lotIndex}" price="80">￥80.00</div>`
                num = 0;
            }
...
function timeslot(even) {
    ...
    if (available == 2) {
        ...
        createCookie("payGoBack", 'basketball_booking.html')
        location.href = `choose_ticket_number.html?1=1&dataTime=${dataTime}&dataCourt=${dataCourt}&price=${price}&type=Basketball&date=${date}&num=${num}`
    }
}
