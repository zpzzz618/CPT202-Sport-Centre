function initData(date = null) {
    ...
    const data = { type: 'Badminton', useDate: date }
    ...
    if (number.available == '1'){
        ...
        for (let index = 0; index < 12; index++) {
            ...
            for (let lotIndex = 0; lotIndex < number.court; lotIndex++) {
                ...
                content += `<div class="${className}" click="timeslot()" data-time="${time}:00" num="${num}" available="${available}" data-court="${lotIndex}" price="35">￥35.00</div>`
                num = 0;
            }
            newPair.innerHTML = content
...
function timeslot(even) {
    ...
    if (available == 2) {
        ...
        createCookie("payGoBack", 'badminton_booking.html')
        location.href = `choose_ticket_number.html?1=1&dataTime=${dataTime}&dataCourt=${dataCourt}&price=${price}&type=Badminton&date=${date}&num=${num}`
    }
}
