function initData(date = null) {
    ...
    const data = { type: 'Swim', useDate: date }
    ...
    if (number.available == '1'){
        for (let index = 0; index < 12; index++) {
            ...
            for (let lotIndex = 0; lotIndex < number.court; lotIndex++) {
                ...
                let available = 2;
                content += `<div class="${className}" click="timeslot()" data-time="${time}:00" num="${num}" available="${available}" data-court="${lotIndex}" price="30">￥30.00</div>`
                num = 0;
            }
...
function timeslot(even) {
    ...
    if (available == 2) {
        ...
        createCookie("payGoBack", 'swim_booking.html')
        location.href = `choose_ticket_number.html?1=1&dataTime=${dataTime}&dataCourt=${dataCourt}&price=${price}&type=Swim&date=${date}&num=${num}`
    }
}
